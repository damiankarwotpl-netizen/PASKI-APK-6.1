...
class FutureApp(App):
    ...
    def special_send_step_2_recipients(self, file_path):
        ...
        def refresh(v=""):
            ...
            for n, s, e in rows:
                ...
                cb = CheckBox(size_hint_x=None, width=dp(50)); cb.bind(active=lambda inst, val, m=e: [self.selected_emails.append(m) if val else self.selected_emails.remove(m)])
        ...
        btn = PremiumButton(text="DALEJ (KROK 3)"); btn.bind(on_press=lambda x: [p.dismiss(), self.special_send_step_3_msg(file_path)] if self.selected_emails else self.msg("!", "Wybierz kogoś!")); box.add_widget(btn); p = Popup(title="Wybór odbiorców", content=box, size_hint=(0.95,0.9)); p.open()

    def special_send_step_3_msg(self, file_path):
        box = BoxLayout(orientation="vertical", padding=dp(15), spacing=dp(10))
        box.add_widget(Label(text="KROK 3: TREŚĆ MAILA", color=COLOR_PRIMARY, size_hint_y=None, height=dp(40)))
        ti_s = TextInput(hint_text="Temat maila", size_hint_y=None, height=dp(50), multiline=False); ti_b = TextInput(hint_text="Treść maila...", multiline=True)
        box.add_widget(ti_s); box.add_widget(ti_b); btn = PremiumButton(text="WYŚLIJ (KROK 4)")
        btn.bind(on_press=lambda x: [p.dismiss(), self.special_send_step_4_progress(file_path, self.selected_emails, ti_s.text, ti_b.text)] if ti_s.text and ti_b.text else self.msg("!", "Wpisz dane!")); box.add_widget(btn); p = Popup(title="Wiadomość", content=box, size_hint=(0.95,0.85)); p.open()
    ...
    def special_send_step_4_progress(self, file_path, target_list, subject, body):
        ...
        p = Popup(title="Status wysyłki", content=box, size_hint=(0.85, 0.45), auto_dismiss=False); btn_c.bind(on_press=p.dismiss); box.add_widget(btn_c); p.open()
        def run():
    ...
    def setup_email_ui(self):
        ...
        auto_box.add_widget(Label(text="WYSYŁKA AUTOMATYCZNA (BEZ POTWIERDZEŃ)", halign="left", text_size=(dp(280), None), font_size='12sp', bold=True))
        ...
        btn("WYŚLIJ PLIK", lambda x: self.open_picker("special_send"))
    ...
    def start_mass_mailing(self, _):
        ...
        self.pb.value = 0; Clock.schedule_once(lambda dt: self.process_mailing_queue())
    ...
    def process_mailing_queue(self, *args):
        ...
        if not self.queue:
            ...
            self.msg("Raport", "Zakończono operację."); return
        ...
        if self.auto_send_mode:
            ...
            else: self.stats["skip"] += 1; Clock.schedule_once(lambda dt: self.process_mailing_queue()); return
    ...
    def send_email_engine(self, row_data, target, fast_mode=False):
        def thread_task():
            ...
            try:
                ...
                msg["Subject"] = (ts[0] if ts else "Wiadomość").replace("{Imię}", nx); msg["From"], msg["To"] = cfg['u'], target; msg.set_content((tb[0] if tb else "Cześć").replace("{Imię}", nx).replace("{Data}", dat))
    ...
    def test_smtp(self, _):
        ...
        if not cfg: self.msg("!", "Zapisz SMTP"); return
        def tk():
            try: srv = smtplib.SMTP("smtp.gmail.com", 587, timeout=10); srv.starttls(); srv.login(cfg['u'], cfg['p']); srv.quit(); Clock.schedule_once(lambda dt: self.msg("OK", "Połączono!"))
    ...
    def refresh_reports(self, *args):
        self.report_grid.clear_widgets(); rows = self.conn.execute("SELECT date, ok, fail FROM reports ORDER BY id DESC").fetchall()
        for d, ok, fl in rows:
            row = BoxLayout(orientation="vertical", size_hint_y=None, height=dp(85), padding=dp(10)); with row.canvas.before: Color(0.15, 0.2, 0.25, 1); r = Rectangle(pos=row.pos, size=row.size); row.bind(pos=lambda i,v,rect=r: setattr(rect, 'pos', v), size=lambda i,v,rect=r: setattr(rect, 'size', v))
    ...
    def refresh_contacts_list(self, *args):
        ...
        for n, s, e, p, ph in rows:
            ...
            row, cb = BoxLayout(size_hint_y=None, height=dp(110), padding=dp(8)), CheckBox(size_hint_x=None, width=dp(50), active=(e in self.selected_emails))
            cb.bind(active=lambda i, v, m=e: [self.selected_emails.append(m) if v else self.selected_emails.remove(m)]); row.add_widget(cb)
            info = BoxLayout(orientation="vertical"); info.add_widget(Label(text=f"{n} {s}".title(), bold=True, halign="left", text_size=(dp(200), None))); info.add_widget(Label(text=f"Email: {e}\nPESEL: {p if p else '-'}", font_size='12sp', color=(0.7,0.7,0.7,1), halign="left", text_size=(dp(200), None))); row.add_widget(info)
            acts = BoxLayout(size_hint_x=None, width=dp(90), orientation="vertical", spacing=dp(4)); acts.add_widget(Button(text="Edytuj", on_press=lambda x, d=(n,s,e,p,ph): self.form_contact(*d))); acts.add_widget(Button(text="Usuń", background_color=(0.7,0.0,0.0,1), on_press=lambda x, name=n, sur=s: self.delete_contact(name,sur))); row.add_widget(acts); self.c_list.add_widget(row)
