...
class FutureApp(App):
    def build(self):
        ...
        # Inicjalizacja środowiska Android
        if platform == "android":
    ...
    def init_db(self):
        db_p = Path(self.user_data_dir) / "future_ultimate_v12.db"
    ...
    def setup_all_ui(self):
        ...
        l.add_widget(Label(text="FUTURE ULTIMATE v12", font_size='34sp', bold=True, color=COLOR_PRIMARY))
    ...
    def mailing_worker(self):
        ...
        except: return self.finish_mailing("Błąd pliku SMTP")
        
        b_on, b_sz, proc = cfg.get('batch', True), 30, 0
        try:
    ...
    def send_single_email(self, srv, cfg, row_data, target):
        try:
            ...
            tmp = Path(self.user_data_dir) / f"temp_{nx}.xlsx"; wb = Workbook(); ws = wb.active
    ...
    def setup_smtp_ui(self):
        l = BoxLayout(orientation="vertical", padding=dp(25), spacing=dp(8)); p = Path(self.user_data_dir) / "smtp.json"
        try: d = json.load(open(p))
        except: d = {}
        self.ti_shost = ModernInput(hint_text="Serwer", text=d.get('h','smtp.gmail.com')); self.ti_sport = ModernInput(hint_text="Port", text=str(d.get('port',587))); self.ti_suser = ModernInput(hint_text="Login", text=d.get('u','')); self.ti_spass = ModernInput(hint_text="Hasło", password=True, text=d.get('p',''))
        ...
        l.add_widget(ModernButton(text="ZAPISZ", on_press=lambda x: [self.save_cfg(p), self.msg("OK","Zapisano")])); l.add_widget(ModernButton(text="TEST POŁĄCZENIA", on_press=lambda x: self.test_smtp_direct(), bg_color=(.1,.7,.4,1))); l.add_widget(ModernButton(text="POWRÓT", on_press=lambda x: setattr(self.sm, 'current', 'home'), bg_color=(.3,.3,.3,1))); self.sc_ref["smtp"].add_widget(l)

    def save_cfg(self, p):
        with open(p, "w") as f: json.dump({'h':self.ti_shost.text,'port':self.ti_sport.text,'u':self.ti_suser.text,'p':self.ti_spass.text,'batch':self.cb_batching.active}, f)

    def test_smtp_direct(self):
        try: srv = self.connect_smtp({'h':self.ti_shost.text,'port':self.ti_sport.text,'u':self.ti_suser.text,'p':self.ti_spass.text}); srv.quit(); self.msg("OK", "Serwer OK")
    ...
    def process_excel(self, path):
        ...
        except: self.msg("BŁĄD", "Uszkodzony plik")
    ...
    def export_single_row(self, r):
        ...
        self.style_xlsx(ws); wb.save(p/f"Raport_{nx}_{sx}.xlsx"); self.msg("OK", f"Eksport zakończony")
    ...
    def start_mass_mailing(self, _):
        ...
        self.total_q = len(self.queue); self.is_mailing_running = True; threading.Thread(target=self.mailing_worker, daemon=True).start()
    ...
    def setup_tmpl_ui(self):
        ...
        l.add_widget(ModernButton(text="ZAPISZ", on_press=lambda x: [self.conn.execute("INSERT OR REPLACE INTO settings VALUES (?,?)", ('t_sub',ti_s.text)), self.conn.execute("INSERT OR REPLACE INTO settings VALUES (?,?)", ('t_body',ti_b.text)), self.conn.commit(), self.msg("OK","Zapisano")]))
