...
class FutureApp(App):
    ...
    def process_mailing_queue(self):
        ...
        # Próba dopasowania po PESEL (funkcja z 67)
        ...
        def thread_task():
            ...
            try:
                ...
                if not fast_mode and self.full_data:
                    tmp = Path(self.user_data_dir) / f"r_tmp.xlsx"; wb = Workbook(); ws = wb.active
    ...
    def setup_ui(self):
        ...
        # Pozostałe UI (Setup z 65 z fixami z 67)
    ...
    def setup_contacts_ui(self):
        ...
        foot.add_widget(PremiumButton(text="WYŚLIJ MULTI (SZYBKO)", on_press=self.run_quick_send))
    ...
    def refresh_contacts_list(self, *args):
        ...
        for n, s, e, p, ph in rows:
            ...
            info.add_widget(SafeLabel(text=f"{e} | PESEL: {p}", font_size='12sp', color=(0.7,0.7,0.7,1), halign="left"))
            row.add_widget(info)
    ...
    def setup_smtp_ui(self):
        ...
        def sv(x): [json.dump({'u':self.ti_su.text, 'p':self.ti_sp.text}, open(Path(self.user_data_dir)/"smtp.json", "w")), self.msg("OK", "Zapisano SMTP")]
        l.add_widget(Label(text="USTAWIENIA SMTP", bold=True)); l.add_widget(ti_su := self.ti_su); l.add_widget(ti_sp := self.ti_sp)
    ...
    def setup_tmpl_ui(self):
        ...
        l.add_widget(Label(text="TREŚĆ WIADOMOŚCI", bold=True)); l.add_widget(self.ti_ts); l.add_widget(self.ti_tb); l.add_widget(PremiumButton(text="ZAPISZ", on_press=sv_tmpl)); l.add_widget(PremiumButton(text="POWRÓT", on_press=lambda x: setattr(self.sm, 'current', 'email'), background_color=(0.4,0.4,0.4,1))); self.screens["tmpl"].add_widget(l)
...
# ===============================
# FUTURE 16.0 PRO – STABILITY PATCH
# ===============================
...
def format_workbook(wb):
    ...
    try:
        ...
        print("Formatowanie Excel error:", e)


def safe_load_excel(path):
    """
    Bezpieczne ładowanie pliku Excel
    """
    try:
        ...
        if ext == ".xlsx":
            ...
            if xlrd is None:
                raise Exception("Brak biblioteki xlrd")

            wb = Workbook()
            ws = wb.active
            ws.append(["Import XLS"])

        else:
            raise Exception("Nieobsługiwany format")
...
def safe_save_excel(wb, path):
    ...
    try:
        ...
        print("Błąd zapisu Excel:", e)
        traceback.print_exc()


# ===============================
# GLOBAL APK CRASH PROTECTION
# ===============================
...
print("Future 16.0 PRO Stability Patch loaded")
