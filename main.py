...
class FutureApp(App):
    ...
    def init_ui(self):
        ...
        self.search = TextInput(hint_text="Szukaj osoby...", multiline=False)
        ...
        lt.add_widget(top)
        lt.add_widget(self.scroll)
        lt.add_widget(self.progress)
    ...
    def setup_settings_uis(self):
        ...
        if p.exists():
            d = json.load(open(p))
            self.s_user.text, self.s_pass.text = d.get('u',''), d.get('p','')
        ls.add_widget(Label(text="Konfiguracja SMTP"))
        ls.add_widget(self.s_user)
        ls.add_widget(self.s_pass)
        ...
        self.ts = TextInput()
        self.tb = TextInput(multiline=True)
        ...
        lt.add_widget(Label(text="Temat i Treść ({Imię}, {Data})"))
        lt.add_widget(self.ts)
        lt.add_widget(self.tb)
    ...
    def pick_file(self, mode):
        ...
        def on_activity_result(request_code, result_code, intent_data):
            ...
            try:
                ...
                if mode == "data": filename = "data_v11.xlsx"
                elif mode == "book": filename = "book_v11.xlsx"
                else: filename = f"extra_{os.urandom(2).hex()}"
                ...
                if mode == "data":
                    ...
                    Clock.schedule_once(lambda x: setattr(self.home_status, "text", "Excel załadowany"))
                elif mode == "book":
                    Clock.schedule_once(lambda x: self.import_contacts_to_db(local_path))
                elif mode == "extra" or mode == "extra_pick":
                    if str(local_path) not in self.global_attachments:
    ...
    def go_to_table(self, _):
        ...
        try:
            ...
            self.show_table(self.full_data)
    ...
    def show_table(self, data):
        ...
        if not data: return
        r_count, c_count = len(data), len(data[0])
        w, h = dp(160), dp(42)
        self.grid.cols = c_count + 1
        self.grid.width, self.grid.height = (c_count + 1) * w, r_count * h
        # Header
        for h_val in data[0]:
            self.grid.add_widget(Label(text=str(h_val), size_hint=(None, None), size=(w, h), bold=True))
    ...
    def start_mailing_thread(self, _):
        ...
        threading.Thread(target=self._mailing_process, args=(False,), daemon=True).start()

    def run_test_mail(self, _):
        if not self.full_data: self.msg("!", "Wczytaj najpierw Excel!"); return
        threading.Thread(target=self._mailing_process, args=(True,), daemon=True).start()
    ...
    def _mailing_process(self, is_test):
        ...
        try:
            ...
            Clock.schedule_once(lambda x: self.msg("Błąd logowania", str(e)))
            return
        ...
        for i, r in enumerate(rows):
            ...
            if target:
                try:
                    ...
                    # Załącznik Excela
                    tmp_p = Path(self.user_data_dir) / "tmp.xlsx"
                    wb = Workbook(); ws = wb.active
                    idxs = self.export_columns if self.export_columns else list(range(len(h)))
                    ws.append([h[k] for k in idxs])
                    ws.append([r[k] for k in idxs])
                    wb.save(str(tmp_p))
                    with open(tmp_p, "rb") as f:
                        msg.add_attachment(f.read(), maintype="application", subtype="xlsx", filename=f"Raport_{r[0]}.xlsx")
        ...
        Clock.schedule_once(lambda x: self.msg("Mailing", f"Ukończono. Wysłano: {sent}"))
    ...
    def mass_export(self, _):
        ...
        def _task():
            ...
            for i, r in enumerate(self.full_data[1:]):
                self.single_export(r, mass=True)
                Clock.schedule_once(lambda dt, p=int(((i+1)/(len(self.full_data)-1))*100): setattr(self.progress, "value", p))
    ...
    def column_selection_popup(self, _):
        ...
        for i, h_val in enumerate(self.full_data[0]):
            ...
            row.add_widget(cb); row.add_widget(Label(text=str(h_val)))
            grid.add_widget(row); chks.append((i, cb))
        def apply(_): self.export_columns = [idx for idx, c in chks if c.active]; p.dismiss()
        sv = ScrollView(); sv.add_widget(grid)
        box.add_widget(sv); box.add_widget(Button(text="ZATWIERDŹ", on_press=apply, size_hint_y=None, height=dp(50)))
        p = Popup(title="Wybierz kolumny raportu", content=box, size_hint=(0.9, 0.9)); p.open()

    def attachment_manager(self, _):
        box = BoxLayout(orientation="vertical", padding=10, spacing=10)
        scroll = ScrollView(); list_box = GridLayout(cols=1, size_hint_y=None); list_box.bind(minimum_height=list_box.setter('height'))
        for ap in self.global_attachments:
            ...
            r.add_widget(btn); list_box.add_widget(r)
        scroll.add_widget(list_box); box.add_widget(scroll)
        box.add_widget(Button(text="DODAJ DOKUMENT", on_press=lambda x: self.pick_file("extra_pick")))
        box.add_widget(Button(text="ZAMKNIJ", on_press=lambda x: p.dismiss()))
        p = Popup(title="Załączniki", content=box, size_hint=(0.8, 0.6)); p.open()
    ...
    def show_history(self, _):
        ...
        self.msg("Logi", txt if txt else "Brak historii wysyłek")

    def update_att_status(self):
        self.att_status.text = f"Załączniki: {len(self.global_attachments)}"
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        with open("critical_crash.txt", "w") as f:
