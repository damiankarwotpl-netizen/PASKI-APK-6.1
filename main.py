...
class PremiumButton(Button):
    def __init__(self, **kwargs):
        ...
        self.background_normal = ""; self.background_color = COLOR_PRIMARY
        self.height = dp(53); self.size_hint_y = None; self.bold = True
...
class FutureApp(App):
    def build(self):
        ...
        self.global_attachments = []; self.selected_emails = [] # Dla szybkiej wysyłki
        self.queue = []; self.stats = {"ok": 0, "fail": 0, "skip": 0, "auto": 0}
...
# -----------------------------
# MASOWA WYSYŁKA (RAPORTY) + PESEL LOGIC
# -----------------------------
    ...
    def process_mailing_queue(self):
        ...
        email = None
        # 1. Próba po PESEL (Wysyłka bez pytania)
        ...
        # 2. Jeśli PESEL zawiódł -> Szukamy po nazwisku i pytamy Popupem
    ...
    def ask_before_send(self, row, email, n, s, pesel):
        ...
        box.add_widget(Label(text="PESEL NIEZGODNY - POTWIERDŹ:", bold=True, color=(1, 0.6, 0.2, 1)))
    ...
    def send_email_engine(self, row_data, target_email, fast_mode=False):
        def thread_task():
            ...
            try:
                ...
                # Tylko dla masowej wysyłki dodajemy raport indywidualny
                if not fast_mode:
                    tmp = Path(self.user_data_dir) / f"tmp_r.xlsx"; wb = Workbook(); ws = wb.active
                ...
                # Załączniki PDF/Obrazy (Naprawa Zrzut 4)
                ...
                if not fast_mode: Clock.schedule_once(lambda d: [setitem(self.stats, "ok", self.stats["ok"]+1), self.process_mailing_queue()])
            except:
                if not fast_mode: Clock.schedule_once(lambda d: [setitem(self.stats, "fail", self.stats["fail"]+1), self.process_mailing_queue()])
        
        def setitem(d, k, v): d[k] = v; return d
        threading.Thread(target=thread_task, daemon=True).start()

# -----------------------------
# SZYBKA WYSYŁKA (BEZ RAPORTU)
# -----------------------------
    def run_quick_send(self, _):
        if not self.selected_emails: self.msg("!", "Zaznacz osoby w bazie!"); return
...
# -----------------------------
# NAPRAWIONE UI: BAZA, TABELA, KOLUMNY
# -----------------------------
    def setup_contacts(self):
        ...
        foot.add_widget(PremiumButton(text="WYŚLIJ PLIK (BEZ RAPORTU)", on_press=self.run_quick_send))
    ...
    def popup_columns(self, _):
        ...
        for i, h in enumerate(self.full_data[0]):
            ...
            r.add_widget(Label(text=txt, halign="left", text_size=(dp(260), None), color=(1,1,1,1))) # Fix Zrzut 2
            gr.add_widget(r)
        sc.add_widget(gr); box.add_widget(sc)
        box.add_widget(PremiumButton(text="ZATWIERDŹ", on_press=lambda x: [setattr(self, 'export_indices', [i for i,c in checks if c.active]), p.dismiss()]))
    ...
    def refresh_table(self):
        ...
        w, h = dp(220), dp(62) # Szerokość kolumn (Fix Zrzut 3)
...
# -----------------------------
# POZOSTAŁE FUNKCJE (PICKER, ARCHIWUM, SMTP)
# -----------------------------
    def setup_ui(self):
        # Home Screen
        ...
        self.ti_search = TextInput(hint_text="Szukaj w tabeli...", multiline=False); self.ti_search.bind(text=self.filter_table)
        mt.add_widget(self.ti_search); mt.add_widget(Button(text="Kolumny", size_hint_x=0.3, on_press=self.popup_columns))
        ...
        # Mailing UI
        self.setup_email_ui(); self.setup_smtp(); self.setup_tmpl()
    ...
    def setup_email_ui(self):
        ...
        btn = lambda t, c: l.add_widget(PremiumButton(text=t, on_press=c))
        btn("📁 IMPORTUJ BAZĘ KONTAKTÓW", lambda x: self.open_picker("book"))
        btn("🔧 ZARZĄDZAJ BAZĄ / SZYBKA WYSYŁKA", lambda x: [self.refresh_contacts_list(), setattr(self.sm, 'current', 'contacts')])
        btn("📝 EDYTUJ TREŚĆ MAILA", lambda x: setattr(self.sm, 'current', 'tmpl'))
        btn("📎 DODAJ ZAŁĄCZNIK", lambda x: self.open_picker("attachment"))
        btn("🚀 MASOWA WYSYŁKA RAPORTÓW", self.start_mass_mailing)
    ...
    def setup_tmpl(self):
        ...
        sv = lambda x: [self.conn.execute("INSERT OR REPLACE INTO settings VALUES (?,?)", (i,j)) for i,j in [('t_sub', self.ti_ts.text),('t_body', self.ti_tb.text)], self.conn.commit(), self.msg("OK", "Szablon zapisany")]
        l.add_widget(Label(text="TREŚĆ WIADOMOŚCI", bold=True)); l.add_widget(self.ti_ts); l.add_widget(self.ti_tb); l.add_widget(PremiumButton(text="ZAPISZ", on_press=sv)); l.add_widget(PremiumButton(text="POWRÓT", on_press=lambda x: setattr(self.sm, 'current', 'email'), background_color=(0.4,0.4,0.4,1))); self.pages["tmpl"].add_widget(l)
